...
class Explorer(Node):
    ...
    def __init__(self):
        ...
        shutdown_on_complete = self.get_parameter('shutdown_on_complete').value
        self.SHUTDOWN_ON_COMPLETE = bool(shutdown_on_complete) if shutdown_on_complete is not None else False
        ...
        self.has_started_mission = False         # Ensure auto-undock happens only once after boot
    ...
    def shutdown_if_configured(self):
        if self.SHUTDOWN_ON_COMPLETE:
            self.get_logger().warn(">> Mission complete and docked. Shutting down node.")
            rclpy.shutdown()
    ...
    def update_dock_status(self, msg):
        ...
        # Initial boot logic: auto-undock only once.
        if self.is_docked and self.state == RobotState.DOCKED and not self.has_started_mission:
            self.has_started_mission = True
            self.start_undocking()

        if self.is_docked and self.state in (RobotState.RETURNING, RobotState.ESCAPING):
            self.set_state(RobotState.COMPLETED, "clean docking after return")
            self.dock_goal_active = False
            self.close_red_cycles = 0
            self.turn_lock_until_ns = 0
            self.send_velocity(0.0, 0.0)
            self.set_leds(0, 255, 0)
            self.shutdown_if_configured()
    ...
    def manual_override_callback(self, msg):
        self.get_logger().info(f">> MODE COMMAND: {msg.data}")
        if msg.data == 'TELEOP' and self.state != RobotState.MANUAL:
            self.get_logger().warn(">> TELEOP OVERRIDE: Human is taking control")
            self.saved_state = self.state
            self.set_state(RobotState.MANUAL, "manual override")
            self.set_leds(0, 0, 255)
            self.send_velocity(0.0, 0.0)
        elif msg.data == 'AUTO' and self.state == RobotState.MANUAL:
            self.get_logger().info(">> RESUMING AUTO MODE")
            # Do not resume into COMPLETED. Keep robot idle if mission is done.
            if self.saved_state == RobotState.COMPLETED:
                self.set_state(RobotState.COMPLETED, "AUTO ignored after completion")
            else:
                self.set_state(self.saved_state, "auto resume")
            self.set_leds(0, 255, 0)
    ...
    def hazard_check(self, msg):
        ...
        if any(detection.header.frame_id.startswith('bump') for detection in msg.detections):
            ...
            self.set_state(RobotState.ESCAPING, "bumper event")
    ...
    def ir_logic(self, msg):
        ...
        if self.state == RobotState.ESCAPING:
            if time.time() < self.escape_end_time:
                ...
                return
            self.set_state(self.pre_escape_state, "escape complete")
    ...
    def start_undocking(self):
        self.set_state(RobotState.UNDOCKING, "undock requested")
        self.dock_goal_active = False
    ...
    def undock_result(self, future):
        self.set_state(RobotState.EXPLORING, "undock complete")
